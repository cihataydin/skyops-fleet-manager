import { Controller, Get, HttpCode, HttpStatus, Inject, Query, ParseIntPipe, DefaultValuePipe } from '@nestjs/common';
...
export class ReportController {
  ...
  @ApiQuery({ name: 'daysThreshold', required: false, type: Number, description: 'Number of days to check for upcoming maintenance alerts (default: 7)' })
  @ApiResponse({
  ...
  public async getFleetHealthReportAsync(
    @Query('daysThreshold', new DefaultValuePipe(7), ParseIntPipe) daysThreshold: number,
  ) {
    const report = await this.reportService.getFleetHealthReportAsync(daysThreshold);
