...
export class MissionService implements IMissionService {
  public constructor(
    ...
    @Inject(forwardRef(() => DRONE_SERVICE_TOKEN))
  ...
  public async getMissionAsync(id: string): Promise<GetMissionResponseDto> {
    const missionCache = await this.cacheService.getAsync<Mission>(
      `mission_${id}`,
    );
  ...
  public async updateMissionAsync(
    ...
    if (mission.status !== MissionStatus.PLANNED) {
      throw new BadRequestException(
        "You can only update the details of a 'PLANNED' mission. Once started or completed, its history is immutable.",
      );
    }
  ...
  public async startMissionAsync(
    id: string,
  ): Promise<UpdateMissionResponseDto> {
    const updatedMission = await this.processMissionChangeAsync(
      id,
