...
export class Migrate1785013660110 implements MigrationInterface {
  name = 'Migrate1785013660110';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "drones" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP(3) WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP(3) WITH TIME ZONE DEFAULT now(), "deleted_at" TIMESTAMP(3) WITH TIME ZONE, "serial_number" character varying(13) NOT NULL, "model" "public"."drones_model_enum" NOT NULL, "status" "public"."drones_status_enum" NOT NULL DEFAULT 'AVAILABLE', "total_flight_hours" numeric(10,2) NOT NULL DEFAULT '0', "last_maintenance_date" TIMESTAMP WITH TIME ZONE, "next_maintenance_due_date" TIMESTAMP WITH TIME ZONE NOT NULL, CONSTRAINT "UQ_c58fb0d50272dda6c64e7ee3acf" UNIQUE ("serial_number"), CONSTRAINT "PK_3137fc855d37186eeccd193569f" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."missions_type_enum" AS ENUM('WIND_TURBINE_INSPECTION', 'SOLAR_PANEL_SURVEY', 'POWER_LINE_PATROL')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."missions_status_enum" AS ENUM('PLANNED', 'PRE_FLIGHT_CHECK', 'IN_PROGRESS', 'COMPLETE', 'ABORTED')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "maintenance_logs" ADD CONSTRAINT "FK_fba8bd6c2957f15780d933c3570" FOREIGN KEY ("drone_id") REFERENCES "drones"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
