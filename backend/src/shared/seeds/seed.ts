...
async function bootstrap() {
  ...
  const maintenanceRepo = app.get<Repository<MaintenanceLog>>(
    getRepositoryToken(MaintenanceLog),
  );

  console.log('Seeding Database...');

  // Clear existing
  await maintenanceRepo.query(
    'TRUNCATE TABLE maintenance_logs, missions, drones CASCADE',
  );
  ...
  for (let i = 1; i <= 55; i++) {
    ...
    const start = new Date(
      Date.now() + (Math.random() * 10 - 5) * 24 * 60 * 60 * 1000,
    ); // Between -5 to +5 days
    const end = new Date(
      start.getTime() + (Math.random() * 5 + 1) * 60 * 60 * 1000,
    ); // 1-6 hours long

    let status = MissionStatus.PLANNED;
    if (start < new Date()) {
      status =
        Math.random() > 0.2 ? MissionStatus.COMPLETED : MissionStatus.ABORTED;
    }
