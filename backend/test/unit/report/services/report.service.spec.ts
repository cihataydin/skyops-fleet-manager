...
describe('ReportService', () => {
  ...
  describe('getFleetHealthReportAsync', () => {
    it('should correctly aggregate data from services to generate a fleet health report', async () => {
      ...
      droneService.getMaintenanceAlertDronesAsync.mockResolvedValue([{ id: 'drone-1' }]);
      ...
      expect(report.statusBreakdown).toEqual({ AVAILABLE: 5, IN_MAINTENANCE: 2, RETIRED: 3 });
      ...
      expect(droneService.getMaintenanceAlertDronesAsync).toHaveBeenCalledWith(3);
      expect(droneService.getAverageFlightHoursAsync).toHaveBeenCalled();
      expect(missionService.getUpcomingMissionsCountAsync).toHaveBeenCalledWith(24);
    });
  });
});
